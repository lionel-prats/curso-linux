...
# Esta línea importa los objetos argv y exit del módulo sys. 
# argv se utiliza para acceder a los argumentos de línea de comandos
# exit se usa para salir del programa con un código de estado específico.

# argv es una lista en Python que contiene los argumentos de línea de comandos pasados al script en ejecución. 
# El nombre argv proviene de "argument vector". 
# Cuando ejecutas un script de Python desde la línea de comandos y pasas argumentos, estos argumentos se almacenan en argv.

 
def print_usage():
        """
                Print the usage string if script was used improperly
                esta funcion saca por pantalla el uso, el modo de uso de este script que es proporcionandole el pid de un proceso (<pid>)
                proporcionandole la cadena de texto que quieres que lea, de memoria RAM (<string to read>)
                la cadena de texto que quieres que utilize para sobreescribir la cadena original (<string to write>)
        """
        print('Usage: \
...
def read_write_heap(pid, read_str, write_str):
    ...
    # calculo el resto entre el largo de la cadena a reemplazar y la cadena de reemplazo 
    diff = len(read_str) - len(write_str) + 1
    
    # si el resto calculado es > 0, significa que el string de reemplazo es mas corto, en cuyo caso completo el string de reemplazo con " " hasta alcanzar el largo del string a reemplazar  
    if diff > 0:
        write_str += ' '*diff
    mem_file.write(bytes(write_str, "ASCII"))

# verifico si hay exactamente cuatro argumentos pasados al script. 
# Si es así, el script asume que se han proporcionado los argumentos correctos y procede a realizar alguna operación
if (len(argv) == 4):
        pid = argv[1]
        search_str = argv[2]
        replace_str = argv[3]
